import pynput.mouse
# import pynput.keyboard # Removed for mouse-only recording
import time
import json
import argparse
import threading
...
# keyboard_listener_global = None # Removed for mouse-only recording
...
# For Ctrl+Shift+Q detection (now for stop/save) # Removed for mouse-only recording
# Using a set to store currently pressed modifier keys for more robust hotkey detection # Removed for mouse-only recording
# pressed_keys_global = set() # Removed for mouse-only recording

# For hotkey suppression (when Ctrl+Shift+Q is used to stop) # Removed for mouse-only recording
# hotkey_was_triggered_flag = False # Removed for mouse-only recording
...
# def get_key_repr(key): # Removed for mouse-only recording
#     if isinstance(key, pynput.keyboard.KeyCode): return key.char
#     elif isinstance(key, pynput.keyboard.Key): return key.name
#     return None

# Hotkey logic (Ctrl+Shift+Q to stop) will be in on_key_press. # Removed for mouse-only recording
# Pause/resume functionality is removed. # Removed for mouse-only recording

# def on_key_press(key): # Removed for mouse-only recording
    ...
    # is_q_for_hotkey = (key_name == 'q' or key_name == 'Q')
    ...
    #     hotkey_was_triggered_flag = True
...
# def on_key_release(key): # Removed for mouse-only recording
    # global pressed_keys_global, recording_start_time_global, stop_recording_event_global, hotkey_was_triggered_flag
    # key_name = get_key_repr(key)
...
def save_and_cleanup(signal_num=None, frame=None):
    ...
    # global mouse_listener_global, keyboard_listener_global, _cleanup_has_run # keyboard_listener_global removed
    global mouse_listener_global, _cleanup_has_run
    ...
    # if keyboard_listener_global and keyboard_listener_global.is_alive(): # Removed for mouse-only recording
    #     try:
    ...
    # if keyboard_listener_global: # Removed for mouse-only recording
...
def start_recording_foreground(output_f, duration_seconds): # Added duration
    ...
    # global mouse_listener_global, keyboard_listener_global, stop_recording_event_global, _cleanup_has_run # keyboard_listener_global removed
    global mouse_listener_global, stop_recording_event_global, _cleanup_has_run
    global remaining_time_global, timer_thread_global # Timer globals
    # global pressed_keys_global, hotkey_was_triggered_flag # Hotkey globals (pause globals removed) # Removed for mouse-only
    ...
    # Initialize/Reset hotkey state for a new recording # Removed for mouse-only
    # pressed_keys_global.clear() # Removed for mouse-only
    # hotkey_was_triggered_flag = False # Removed for mouse-only
    ...
    try:
        ...
        # print("Press Ctrl+C or Ctrl+Shift+Q to stop recording, or run 'python3 mouse_recorder_v2.py stop' from another terminal.") # Ctrl+Shift+Q removed
        print("Press Ctrl+C to stop recording, or run 'python3 mouse_recorder_v2.py stop' from another terminal.")
    except IOError as e:
    ...
    try:
        ...
        # keyboard_listener_global = pynput.keyboard.Listener(on_press=on_key_press, on_release=on_key_release) # Removed for mouse-only
        print("DEBUG: pynput mouse listener object created.", file=sys.stderr, flush=True)
    except Exception as e_listener_init:
        print(f"DEBUG: ERROR INITIALIZING PYNPUT MOUSE LISTENER: {e_listener_init}", file=sys.stderr, flush=True)
        import traceback
    ...
    try:
        ...
        # keyboard_listener_global.start() # Removed for mouse-only
        # print("DEBUG: Keyboard listener started.", file=sys.stderr, flush=True) # Removed for mouse-only
    except Exception as e_listener_start:
        # print(f"DEBUG: ERROR STARTING PYNPUT LISTENERS: {e_listener_start}", file=sys.stderr, flush=True) # Modified message
        print(f"DEBUG: ERROR STARTING PYNPUT MOUSE LISTENER: {e_listener_start}", file=sys.stderr, flush=True)
        import traceback
        traceback.print_exc(file=sys.stderr)
        if mouse_listener_global and mouse_listener_global.is_alive(): mouse_listener_global.stop()
        # if keyboard_listener_global and keyboard_listener_global.is_alive(): keyboard_listener_global.stop() # Removed for mouse-only
        if not _cleanup_has_run: save_and_cleanup()
        sys.exit(1)
    ...
    try:
        ...
        # if keyboard_listener_global and keyboard_listener_global.is_alive(): # Removed for mouse-only
...
def main():
    parser = argparse.ArgumentParser(description="Mouse Recorder (Foreground)") # Modified description
    subparsers = parser.add_subparsers(dest="command", required=True)
    start_parser = subparsers.add_parser("start", help="Start mouse recording (blocks terminal).") # Modified help
